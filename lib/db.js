...
export async function dbInsertOrder(data) {
  ...
  const existing = await redis.hget("orders", key);
  const record = existing
    ? { ...existing, ...data }
    : { id: Date.now(), created_at: new Date().toISOString(), ...data };
  await redis.hset("orders", { [key]: record });
}
...
export async function dbDeleteOrder(id) {
  // Essaie par order_id d'abord
  const exists = await redis.hexists("orders", id);
  if (exists) { await redis.hdel("orders", id); return; }
  // Sinon cherche par id numérique
  const all = await redis.hgetall("orders");
  if (!all) return;
  const entry = Object.entries(all).find(([, v]) => String(v.id) === String(id));
  if (entry) await redis.hdel("orders", entry[0]);
}
