...
export async function dbUpsertCheckout(data) {
  const checkouts = await get("checkouts");
...
export async function dbDeleteCheckout(id) {
  const checkouts = await get("checkouts");
  await set("checkouts", checkouts.filter(c => String(c.id) !== String(id)));
}
...
export async function dbGetLive() {
  ...
  const pageViewMap = {};
  for (const p of pageviews.filter(pv => pv.created_at > cutoff24h)) {
    const key = p.path || p.page || "/";
    pageViewMap[key] = (pageViewMap[key] || 0) + 1;
  }
  const pageStats = Object.entries(pageViewMap)
    .map(([page, count]) => ({ page, count }))
    .sort((a, b) => b.count - a.count);

  // Taux de conversion panier / checkout sur 24h
  const checkouts24h = checkouts.filter(c => (c.updated_at || c.created_at) > cutoff24h);
  const atc24h     = checkouts24h.length;
  const payment24h = checkouts24h.filter(c => c.step === "payment_initiated" || c.completed).length;

  return { count: active.length, pages, sessions: active, recent_atc, recent_pageviews, visits24h, pageStats, atc24h, payment24h };
}
...
export async function dbGetStats() {
  ...
  const cutoff24h  = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  const visits24h  = pageviews.filter(p => p.created_at > cutoff24h).length;
  const chk24h     = checkouts.filter(c => (c.updated_at || c.created_at) > cutoff24h);
  const atc24h     = chk24h.length;
  const payment24h = chk24h.filter(c => c.step === "payment_initiated" || c.completed).length;
  ...
  return { revenue, orders: paid.length, visits, abandoned, converted, chart: days, visits24h, atc24h, payment24h };
}
